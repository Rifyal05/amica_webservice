...
def get_my_bot_chat(user_id):
    chat = BotChat.query.filter_by(user_id=user_id).first()
    if not chat:
        chat = BotChat(user_id=user_id) # type: ignore
        db.session.add(chat)
        db.session.commit()
    return chat
...
def get_bot_history():
    ...
    if not chat:
        return jsonify([])
    messages = BotMessage.query.filter_by(bot_chat_id=chat.id).order_by(desc(BotMessage.sent_at)).limit(50).all()
    results = []
    for msg in messages:
