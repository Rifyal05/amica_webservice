...
def upgrade() -> None:
    # Memastikan extension pgcrypto tersedia untuk gen_random_uuid()
    op.execute('CREATE EXTENSION IF NOT EXISTS "pgcrypto";')

    # Eksekusi SQL Script Lengkap
    op.execute("""
    -- TABLE USERS
...
def downgrade() -> None:
    # Hapus semua jika perlu rollback
    op.execute("""
    DROP TABLE IF EXISTS audit_logs, blocked_users, messages, chat_participants, chats, 
    appeals, reports, sdq_results, feedback, articles, post_likes, seen_posts, 
    saved_posts, connections, comments, posts, users CASCADE;
    """)